...
while True:
    ...
    if 'doordash' in food.get_current_url() or "ubereats" in food.get_current_url() or "grubhub" in food.get_current_url():
        pg = requests.get('https://sky.shiiyu.moe/stats/CoolDabber/Kiwi')
        ...
        if int(lvl)>=18:
            ...
            food.go_to('google.com')
            food.execute_script(f"document.getElementsByTagName('body')[0].innerHTML='FARMER JOE:You only have {lvl} out of 18 farming levels required to eat';")
